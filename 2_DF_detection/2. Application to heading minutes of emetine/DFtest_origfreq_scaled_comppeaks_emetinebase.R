...
# 4) Apply to the first few minutes of emetine recordings, that could be referred to as baseline.


rm(list=ls())


out2.dir = "..." # directory to save resutls 
...
DF_detect = function(filename,min){
...
#animals = c("13-18","14-18","15-18","16-18","29-18","32-18","43-17")
#trials  = paste0(animals,"_Trial",c("15","19","20","07","02","02","09"),"_chpaddle")
#basemins = c(5,1,5,7,1,0,1)


animals = c("13-18","15-18","16-18")
trials  = paste0(animals,"_Trial",c("15","20","07"),"_chpaddle")
basemins = c(5,5,7)
...
for(i.animal in 1:length(animals)){
  ...
  for(i in 1:length(animal_spec)){
    result = rbind(result,DF_detect(animal_spec[i],basemins[i.animal]))
  }
  print(animals[i.animal])
}

write.csv(result,file.path(out2.dir,"DFtest_origfreq_scaled_comppeaks_emetinebase.csv"),na="",row.names = F)
